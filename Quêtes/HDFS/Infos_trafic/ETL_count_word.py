...
class counts_words(MRJob):

    # Select only the words on the corps column
    def mapper(self, _, line):
        try:
            (id,debut,fin,maj,status,cause,severite,code,type_objet,nom,titre,corps) = line.split(";")
            for word in words.findall(html_words.sub('', corps)):
                yield word.lower(), 1
        except:
            pass
    ...
    def reduce_sort(self, _, word_counts):
        for count, word in sorted(word_counts, reverse=True):
            # I keep only the words with more than 100 occurrences
